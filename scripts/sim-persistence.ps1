...
Write-Host "[+] Task created — waiting 10 seconds for detection..." -ForegroundColor Green
...
# Clean up
Write-Host ""
Write-Host "[*] Cleaning up — deleting task..." -ForegroundColor Yellow
schtasks /delete /tn $taskName /f 2>&1
Write-Host "[+] Task deleted — TaskScheduler should log EID4699 (Task Deleted)" -ForegroundColor Green
...
Write-Host ""
Write-Host "[!] Document in MITRE matrix: T1053.005 — Scheduled Task" -ForegroundColor Magenta
